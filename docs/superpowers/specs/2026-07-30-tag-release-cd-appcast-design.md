...
The public release channel repository is `kwiats/switch-commit-release-channel`. Its GitHub Pages branch should contain the ZIP artifacts, checksums, generated `appcast.xml`, optional Sparkle delta files, and release notes. The app must not point to the private source repository at runtime.
...
- copy the new ZIP, checksum, and release notes into that clone,
- run Sparkle's `generate_appcast` against the release channel directory,
...
- `build-release.sh` derives the appcast URL and artifact URL from the same public base URL,
